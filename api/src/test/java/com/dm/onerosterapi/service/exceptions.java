...
public class exceptions {
    ...
    @Test
    public void UserExceptions() throws
        UserNotFoundException,
        ClassOfCourseNotFoundException,
        SchoolNotFoundException
 {

        try { userService.getUserBySourcedId("x"); } catch (UserNotFoundException e){
            assertTrue(e.getMessage().contains(ApiMessages.NO_USERS_FOR_ID + "x"));
        }

        try { userService.getTeacherBySourcedId(studentId); } catch (UserNotFoundException e){
            assertTrue(e.getMessage().contains(ApiMessages.NOT_A_TEACHER + studentId));
        }

        try { userService.getStudentBySourcedId(teacherId); } catch (UserNotFoundException e){
            assertTrue(e.getMessage().contains(ApiMessages.NOT_A_STUDENT + teacherId));
        }

        try { userService.getUsersByClass("x"); } catch (ClassOfCourseNotFoundException e){
            assertTrue(e.getMessage().contains(ApiMessages.INVALID_CLASS + "x"));
        }

        try { userService.getUsersBySchool("x"); } catch (SchoolNotFoundException e){
            assertTrue(e.getMessage().contains(ApiMessages.INVALID_SCHOOL + "x"));
        }

        try { userService.getUsersForClassInSchool("x", "y"); } catch (ClassOfCourseNotFoundException e){
            assertTrue(e.getMessage().contains(ApiMessages.INVALID_CLASS + "x"));
        }

        try { userService.getUsersForClassInSchool(classId, "y"); } catch (SchoolNotFoundException e){
    ...
    @Test
    public void ClassExceptions() throws
            UserNotFoundException,
            ClassOfCourseNotFoundException
    {

        try { classService.getBySourcedId("x"); } catch (ClassOfCourseNotFoundException e) {
            assertTrue(e.getMessage().contains(ApiMessages.NO_CLASSES_FOR_ID + "x"));
        }

        try { classService.getClassesByTerm("x"); } catch (TermNotFoundException e) {
            assertTrue(e.getMessage().contains(ApiMessages.INVALID_TERM + "x"));
        }

        try { classService.getClassesBySchool("x"); } catch (SchoolNotFoundException e) {
            assertTrue(e.getMessage().contains(ApiMessages.INVALID_SCHOOL + "x"));
        }

        try { classService.getClassesByCourse("x"); } catch (CourseNotFoundException e) {
            assertTrue(e.getMessage().contains(ApiMessages.INVALID_COURSE + "x"));
        }

        try { classService.getClassesByUser("x"); } catch (UserNotFoundException e) {
            assertTrue(e.getMessage().contains(ApiMessages.INVALID_USER + "x"));
        }

        try { classService.getClassesByTeacher(studentId); } catch (UserNotFoundException e) {
            assertTrue(e.getMessage().contains(ApiMessages.NOT_A_TEACHER + studentId));
        }

        try { classService.getClassesByStudent(teacherId); } catch (UserNotFoundException e) {
            assertTrue(e.getMessage().contains(ApiMessages.NOT_A_STUDENT + teacherId));
        }
